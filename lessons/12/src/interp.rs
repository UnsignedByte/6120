...
pub fn get_arg<'a, T: From<&'a Value>>(vars: &'a Environment, index: usize, args: &[usize]) -> T {
...
fn execute_value_op(
    ...
    use bril_rs::ValueOps::{
        Add, Alloc, And, Bits2Float, Call, Ceq, Cge, Cgt, Char2int, Cle, Clt, Div, Eq, Fadd, Fdiv,
        Feq, Fge, Fgt, Fle, Float2Bits, Flt, Fmul, Fsub, Ge, Get, Gt, Id, Int2char, Le, Load, Lt,
        Mul, Not, Or, PtrAdd, Sub, Undef,
    };
    match op {
        Add => {
        ...
        Call => {
            ...
            let ExecutionResult::Res(result) = execute(state, callee_func, trace)? else {
                state.env.pop_frame();
                return Ok(());
            };
...
fn execute_effect_op(
    ...
    match op {
        ...
        Print => {
            // No need to do anything here, this interpreter doesn't support printing
        }
        Nop => {}
        Call => {
            let callee_func = state.prog.get(funcs[0]).unwrap();
            ...
            execute(state, callee_func, trace)?;
            state.env.pop_frame();
        }
        Store => {
            let arg0 = get_arg::<&Pointer>(&state.env, 0, args);
...
fn execute<'a>(
    ...
    func: &'a BBFunction,
    trace: &mut Trace,
) -> Result<ExecutionResult, PositionalInterpError> {
    ...
    loop {
        ...
        for (curr_instr_idx, (code, numified_code)) in curr_instrs
            .iter()
            .zip(curr_numified_instrs.iter())
            .enumerate()
        {
            eprintln!(
                "Executing instruction @{}, {}, {}: {}",
                &func.name, curr_block_idx, curr_instr_idx, code
            );

            if trace.push(code, numified_code, state).is_err() {
                eprintln!("Tracing errored");
                return Ok(ExecutionResult::Done(curr_block_idx, curr_instr_idx));
            }
            ...
            // If the trace is done, we can also return here. This is to handle if we finish inside a function
            if trace.done() {
                return Ok(ExecutionResult::Done(curr_block_idx, curr_instr_idx));
            }
        }
        ...
        if let Some(idx) = next_block_idx {
            ...
            return Ok(ExecutionResult::Res(result));
        }
    }
}
...
// Traceable modification of state
pub struct State<'a> {
    pub prog: &'a BBProgram,
    pub env: Environment,
    pub heap: Heap,
...
impl<'a> State<'a> {
    const fn new(prog: &'a BBProgram, env: Environment, heap: Heap) -> Self {
        Self {
            prog,
            env,
            heap,
...
/// Copy of brilirs::execute_main that traces the first `trace_length` instructions
pub fn execute_main(
    prog: &BBProgram,
    trace_length: usize,
    input_args: &[String],
) -> Result<(Trace, usize, usize), PositionalInterpError> {
    let main_func = prog
        .index_of_main
    ...
    let mut trace = Trace::new(trace_length);

    let (bbidx, iidx) = match execute(&mut state, main_func, &mut trace)? {
        ExecutionResult::Done(bbidx, iidx) => (bbidx, iidx),
        ExecutionResult::Res(_) => {
            // Trace reached the end of the program
            (
                main_func.blocks.len() - 1,
                main_func.blocks.last().unwrap().instrs.len(),
            )
        }
    };
